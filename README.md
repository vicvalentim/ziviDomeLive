...
Standard draws an observer-centred sky sphere in camera space and converts its directions back to world space through the inverse camera basis. Spherical modes use `SphericalOrientation` during canonical cubemap capture. When a scene applies the shared `getSceneCamera()` transform, its quaternion is composed into both Environment paths automatically; camera target and distance are deliberately ignored so the background remains infinite and translation-invariant. The shaders perform base-level bilinear sampling with horizontal longitude repeat and vertical pole clamp, leaving the borrowed Processing texture state untouched.
