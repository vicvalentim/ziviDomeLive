...
// Package-private implementation grouped physically under _internal/performance.
...
final class PerformanceMonitor {
    ...
    synchronized void activateGpu(
        ...
        activateGpu(expectedSessionId, backend, architecture, null);
    }

    /**
     * Marks a qualified GPU measurement backend active and retains its qualification diagnostic.
     */
    synchronized void activateGpu(
            long expectedSessionId,
            GpuTimerBackend backend,
        ...
        if (requestedMode == PerformanceMode.CPU_GPU && isSessionActive(expectedSessionId)) {
            ...
            gpuTimerBackend = backend == null ? GpuTimerBackend.NONE : backend;
            gpuTimerArchitecture = architecture == null
                    ? GpuTimerArchitecture.OTHER
                    : architecture;
    ...
    /** Falls back truthfully after a capability, qualification, or runtime failure. */
    ...
    public synchronized PerformanceSnapshot snapshot() {
        ...
        List<String> diagnostics = new ArrayList<>(6);
        if (requestedMode == PerformanceMode.CPU_GPU && effectiveMode == PerformanceMode.CPU) {
            diagnostics.add(gpuDiagnostic == null
                    ? "CPU_GPU requested; no qualified GPU measurement backend is available, so CPU profiling is active."
                    : gpuDiagnostic);
        }
        if (effectiveMode == PerformanceMode.CPU_GPU) {
            if (gpuTimerBackend == GpuTimerBackend.FENCE_COMPLETION) {
                diagnostics.add("GPU completion latency uses FENCE_COMPLETION on "
                        + gpuTimerArchitecture
                        + " and covers RENDER_PIPELINE only. It includes CPU-observed queue/completion latency and must not be interpreted as timer-query GPU execution time; all other timings are CPU wall time.");
            } else {
                diagnostics.add("GPU execution time uses qualified " + gpuTimerBackend
                        + " on " + gpuTimerArchitecture
                        + " and covers RENDER_PIPELINE only; all other timings are CPU wall time.");
            }
            if (gpuDiagnostic != null && !gpuDiagnostic.isBlank()) {
                diagnostics.add(gpuDiagnostic);
            }
        }
        if (droppedGpuSamples > 0L) {
            diagnostics.add("Asynchronous GPU samples dropped without blocking: "
                    + droppedGpuSamples + ".");
        }
        if (cubemapCaptureViolations > 0L) {
            diagnostics.add("Cubemap capture invariant violations: "
                    + cubemapCaptureViolations + ".");
        }
        if (unexpectedPassViolations > 0L) {
            diagnostics.add("Render-pass invariant violations: "
    ...
    private static void validateCapacity(int sampleCapacity) {
        if (sampleCapacity < MINIMUM_CAPACITY || sampleCapacity > MAXIMUM_CAPACITY) {
            throw new IllegalArgumentException(
                    "Sample capacity must be between " + MINIMUM_CAPACITY + " and "
                            + MAXIMUM_CAPACITY + ".");
        }
    }
