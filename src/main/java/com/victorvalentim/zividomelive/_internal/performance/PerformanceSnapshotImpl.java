 ...
 * @since 2.0.0
 */
final class PerformanceSnapshotImpl implements PerformanceSnapshot {
	...
	private final PerformanceSnapshot.MetricStatistics[] statistics;
	private final long[][] gpuDurationsNanos;
	private final int[][] gpuCalls;
	private final PerformanceSnapshot.MetricStatistics[] gpuStatistics;
	 ...
	 */
	public PerformanceSnapshot.MetricStatistics getStatistics(PerformanceMetric metric) {
		if (metric == null) {
	 ...
	 */
	public PerformanceSnapshot.MetricStatistics getGpuStatistics(PerformanceMetric metric) {
		if (metric == null) {
			throw new IllegalArgumentException("Performance metric cannot be null.");
		}
	...
	private PerformanceSnapshot.MetricStatistics[] createStatistics() {
		return createStatistics(durationsNanos, calls);
	}

	private PerformanceSnapshot.MetricStatistics[] createStatistics(
			long[][] metricDurations,
			int[][] metricCalls) {
		PerformanceMetric[] metrics = PerformanceMetric.values();
		PerformanceSnapshot.MetricStatistics[] result =
				new PerformanceSnapshot.MetricStatistics[metrics.length];
	...
	private static PerformanceSnapshot.MetricStatistics aggregate(
		...
		return new MetricStatisticsImpl(
	...
	private static final class MetricStatisticsImpl implements PerformanceSnapshot.MetricStatistics {
