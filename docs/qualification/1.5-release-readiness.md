...
- 171 JUnit tests, with 0 failures, 0 errors, and 1 environment-conditioned skip;
...
|---|---|---|
| GPU visual compatibility | CompatibilityLock captures compared with qualified 1.4 reference | Not executed |
| Cubemap face identity/layout | Six labeled faces, rotations, mirroring, and seams | Not executed |
...
1. Confirm `+X`, `-X`, `+Y`, `-Y`, `+Z`, and `-Z` face identity.
2. Capture fisheye, equirectangular, cubemap, and Standard views.
3. Exercise 90-degree pitch, yaw, and roll steps.
4. Verify FOV at representative values including the default `210`.
5. Verify Size% scaling around the image center, including `100` and a reduced calibration value.
6. Run `RenderMode.STANDARD` with floating domemaster enabled.
7. Repeat with each supported native output and receiver.
8. Change output resolution through 1024, 2048, 3072, and 4096.
9. Repeat enable/disable, pause/resume, and terminal shutdown.
10. Record Processing version, OS, CPU architecture, GPU, driver, receiver, resolution, and observed OpenGL errors.
