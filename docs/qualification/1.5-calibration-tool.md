...
projection, focus, color, and output qualification. Scene 1 forms a closed cube
from six face-local GLSL 4.10 targets. Scene 2 maps Paul Bourke's original,
unmodified 8192 x 4096 v14 equirectangular pattern onto a complete 1800-unit
sphere centered at `(0, 0, 0)`, with its north pole at `+X`. The sketch deliberately does not call `ziviDome.draw()` from its `draw()`
...
| Bourke 360-degree map | north and south poles reach `+X` and `-X`; longitude closes without a texture seam | GPU capture |
