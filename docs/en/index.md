...
Version 2.0.0 consolidates the native cubemap renderer. It preserves the Processing-facing `Scene.sceneRender(PGraphicsOpenGL)` contract while replacing the spherical face-array backend with native `GL_TEXTURE_CUBE_MAP` capture and `samplerCube` projection shaders.
